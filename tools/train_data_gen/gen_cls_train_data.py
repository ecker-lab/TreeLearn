...
from tree_learn.util import (get_root_logger, get_detections, make_labels_consecutive, 
                             get_config, get_instances, assign_remaining_points_nearest_neighbor)
...
def get_classifier_training_data(config_cls_data_gen, config_pipeline):
    # logger
    documentation_dir = os.path.join(config_cls_data_gen.base_dir, 'documentation')
    ...
    for key in config_cls_data_gen.pipeline:
        config_pipeline[key] = config_cls_data_gen.pipeline[key]

    # directories
    forest_dir = os.path.join(config_cls_data_gen.base_dir, 'forests')
    save_dir_classifier_data = os.path.join(config_cls_data_gen.base_dir, 'classifier_data')
    ...
    for plot_file in tqdm(os.listdir(forest_dir)):
        ...
        plot_pointwise_results_dir = os.path.join(config_cls_data_gen.base_dir, 'results', plot_name, 'pointwise_gt_and_preds')
        plot_instance_results_dir = os.path.join(config_cls_data_gen.base_dir, 'results', plot_name, 'instance_preds')
        ...
        for radius in config_cls_data_gen.instances.grouping_radii:
            # get instances by grouping based on pointwise predictions
            config_cls_data_gen.grouping.radius = radius
            instance_preds = get_instances(coords, offset_predictions, semantic_prediction_logits, 
                                        config_cls_data_gen.grouping, TREE_CLASS_IN_DATASET, 
                                        global_filtering=config_cls_data_gen.instances.global_filtering, 
                                        local_filtering=config_cls_data_gen.instances.local_filtering)
    ...
    for plot_file in tqdm(os.listdir(forest_dir)):
        ...
        for i, radius in enumerate(config_cls_data_gen.instances.grouping_radii):
            # get instances by grouping based on pointwise predictions
            ...
            mask_positive = max_iou >= config_cls_data_gen.classifier_training_data.iou_threshold_positive[i]
            mask_negative = max_iou <= config_cls_data_gen.classifier_training_data.iou_threshold_negative[i]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_cls_data_gen', type=str, help='path to config file for plot results generation')
    args = parser.parse_args()
    config_pipeline = get_config(args.config_pipeline)
    config_cls_data_gen = get_config(args.config_cls_data_gen)
    get_classifier_training_data(config_cls_data_gen, config_pipeline)
