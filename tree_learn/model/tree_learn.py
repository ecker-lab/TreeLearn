...
class TreeLearn(nn.Module):
    ...
    def forward_backbone(self, coords, feats, batch_ids, batch_size, voxel_sizes, **kwargs):
        voxel_feats, voxel_coords, v2p_map, spatial_shape = voxelize(torch.hstack([coords, feats]), batch_ids, batch_size, voxel_sizes, self.use_coords, self.use_feats, max_num_points_per_voxel=self.max_num_points_per_voxel)
...
class Classifier(nn.Module):
    ...
    def forward(self, batch, return_loss):
        backbone_output, _ = self.forward_backbone(**batch)
    ...
    def get_loss(self, model_output, cls_labels, **kwargs):
        loss_dict = dict()
        weight = torch.ones(2)
...
def voxelize(feats, batch_ids, batch_size, voxel_sizes, use_coords, use_feats, max_num_points_per_voxel, epsilon=1):
